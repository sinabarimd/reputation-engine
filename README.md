...
- **Real headshots wired into the entity schema across all 4 sites** (Jun 27) -- Picked one shot per site to match its voice (navy suit / hub, builder-at-desk / sinabari.net, white coat / sinabariplasticsurgery, half-zip smile / drsinabari). Each site gets a visible portrait, `og:image` + `twitter:image`, and a new `ImageObject` node in the JSON-LD `@graph` referenced by `primaryImageOfPage`. Hub also embeds the YouTube walkthrough as a responsive 16:9 iframe in a new `Watch` section, matching the existing `VideoObject @id #video-reputation-engine` in the entity graph.
- **sinabarimd.com hub rewrite -- entity-anchor + funnel** (Jun 27) -- Earlier the same day: retired the chronological blog feed and the `PIPELINE:PUBLICATIONS` / `PIPELINE:INSIGHTS` auto-sections. Replaced with a static Selected Work router that funnels OUT to sinabari.net (Healthcare AI), drsinabari.com (Essays), and sinabariplasticsurgery.com (Surgical Practice). Hub removed from auto-dispatch; cornerstone content is hand-authored. The Publisher was hardened in the same session to tolerate the new format (hub renderer trimmed, `Replace PIPELINE Section` skips missing markers, `Generate Articles Index` fetches the live "Selected Writing" index for sinabarimd instead of regenerating).
- **4 misplaced topical articles migrated to satellites with 301s** (Jun 27) -- Clinical AI governance + personal AI stack regenerated and published on sinabari.net; value-based care + longevity-medicine regenerated and published on sinabariplasticsurgery.com. Each old hub URL 301s to its specific new satellite article URL via Traefik `redirectRegex` middlewares. All 19 pre-existing satellite articles preserved through the deploys.
- **Instagram carousel format in the spotlight Web 2.0 campaign** (Jun 27) -- Day 13 of the 15-task spotlight campaign is now a 6-slide IG carousel instead of a single post. Each slide is `{ slide_number, image_brief, slide_text }`; caption goes in the task's `content` field. Dashboard renders the slides inline (numbered badges, copy buttons per slide). Fixed a latent `openclaw_key` auth bug in the campaign generator at the same time.
- **Deploy service binary file support** (Jun 27) -- `deploy/deploy_service.py` now accepts a `content_b64` field on the deploy payload alongside the existing `content` (text), so image assets can round-trip through the Publisher's full-file-sync. Used by the new drsinabari portrait preservation block in `Build Deploy Payload`.
